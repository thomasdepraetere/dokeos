...
$htmlHeadXtra[] = '
<style type="text/css">
  .session_course_item .list-middle {
...
echo '<ul class="block__list">';
    echo '<li class="block__listitem"><a href="main/create_course/add_course.php" class="btn--icon"><i class="icon-board"></i> &nbsp;&nbsp;' . get_lang('CourseCreate') . '</a></li>';
...
function display_trainer_links() {
    ...
    //$trainers_manual_link = 'http://www.dokeos.com/wordpress/wp-content/uploads/2014/03/DokeosManagerTrainersManual-v3.pdf';
      $trainers_manual_link = 'http://www.dokeos.com/documentation/';
    if ($user_selected_language == "french") {
        //$trainers_manual_link = 'http://www.dokeos.com/wordpress/wp-content/uploads/2014/03/DokeosManagerTrainersManual-v3.pdf';
	//$trainers_manual_link = 'http://www.dokeos.com/wordpress/wp-content/uploads/2014/11/DOK--ebook.pdf';
	$trainers_manual_link = 'http://www.dokeos.com/fr/documentation/';
    }
...
//    echo "<a href=\"" . $trainers_training_link . "\" target=\"_blank\">" . Display::return_icon('pixel.gif', get_lang('TrainersTraining'), array('class' => 'homepage_button homepage_trainers_training', 'align' => 'middle', 'style' => 'float:left')) . '<div class="tablet_section_left_link">' . get_lang('TrainersTraining') . "</div></a>";
    echo '<li class="block__listitem"><a href="' . $trainers_manual_link . '" target=\"_blank\" class="btn--icon"><i class="icon-pdf"></i> &nbsp;&nbsp;' . get_lang('TrainersManual') . '</a></li>';

//    echo "<a href=\"http://www.dokeos.com/en/video-tutorials/\" target=\"_blank\">" . Display::return_icon('pixel.gif', get_lang('VideoTutorials'), array('class' => 'homepage_button homepage_video_tutorials', 'align' => 'middle', 'style' => 'float:left')) . '<div class="tablet_section_left_link">' . get_lang('VideoTutorials') . "</div></a>";
    echo '<li class="block__listitem"><a href="' . $trainers_training_link . '" target=\"_blank\" class="btn--icon"><i class="icon-group"></i> &nbsp;&nbsp;' . get_lang('TrainersTraining') . '</a></li>';}
echo '</ul>';
/**
...
function get_logged_user_course_html($course, $session_id = 0, $class = 'courses') {
    ...
    $progress = Tracking::get_track_module_student_progress($user_id, $course_system_code, $session_id);
    $duration = Tracking :: get_course_scenario_duration($course_system_code, $session_id);
    ...
    $s_htlm_status_icon = (!empty($s_htlm_status_icon)) ? $s_htlm_status_icon : '<img title="' . get_lang('ImageCourse') . '" src="' . api_get_path(WEB_IMG_PATH) . 'thumbnail-course.png?t=' . time() . '" />';
    //display course entry
    $result.="\n\t";
    $result .= '<div class="' . $class . '" style="position:relative;padding:15px 0px;border-bottom:1px solid #ccc;">';
    ...
    if (!api_is_platform_admin()) {
        ...
        if (empty($date_end_payment)) {
            $query = "SELECT date_end,NOW() AS today FROM " . Database::get_main_table(TABLE_MAIN_PAYMENT_COURSE_REL_USER) . " WHERE user_id='" . $user_id . "' AND course_code='" . $course_system_code . "'";
            $resultPayment = Database::query($query);
            $course_payment = Database::fetch_array($resultPayment);
            $date_end_payment = trim($course_payment['date_end']);
            $date_today = trim($course_payment['today']);
    ...
    if (api_get_setting('use_session_mode') == 'true' && !$nosession) {
        ...
        if ($user_in_course_status == COURSEMANAGER || ($date_start <= $now && $date_end >= $now) || $date_start == '0000-00-00') {
            ...
            $result .= '<div class="listItem__img">' . $s_htlm_status_icon . '</div></a>';
            if ($Closed == '') {
                $result .= '<a href="' . api_get_path(WEB_COURSE_PATH) . $course_directory . '/?id_session=' . $my_course['id_session'] . '">';
            }
            $result .= '<span style="font-size:20px; font-weight:bolder;color:#000;">
                        <strong>' . $course_display_title . '</strong></span></a>
        ...
        $result .= '<span class="coursestatusicons"><div class="thumbnail-left">' . $s_htlm_status_icon . '</div></span>
                        <strong style="font-size:20px; font-weight:bolder;">' . $course_display_title . '</strong>
                    <span style="font-size:15px; font-style:italic; color:#ff3133;"><strong>' . $Closed . '</strong></span>';
        if ($Closed == '')
            $result .= '</a>';
    }
    //}
    ...
    $result .='<div class="list-middle" style="margin-top:5px; margin-bottom:5px;display: table;min-height:70px;">';
    $result .='<span>' . get_lang("With") . ' : </span><span>' . $manager . '</span><br/>';
    
    if (!empty($my_course['id_session'])) {
        $sessionInfo = api_get_session_info($my_course['id_session']);
        if ($sessionInfo['date_start'] != '0000-00-00') {
            $result .='<span>' . get_lang("Begin") . ' : </span><span>' . api_format_date(DATE_FORMAT_SHORT_WITH_LONG_YEAR, strtotime($sessionInfo['date_start'])) . '</span><br/>';
        }
        if ($sessionInfo['date_end'] != '0000-00-00') {
            $result .='<span>' . get_lang("End") . ' : </span><span>' . api_format_date(DATE_FORMAT_SHORT_WITH_LONG_YEAR, strtotime($sessionInfo['date_end'])) . '</span><br/>';
        }
    }
    
    $result .='<span>' . get_lang("ScoreF2F") . ' : </span><span>' . $percentageScore . '</span><br/>';
    $result .='<span>' . get_lang("Progress") . ' : </span><span>' . $progress_t . '</span><br/>';
    if ($duration <> 0) {
	$result .='<span>' . get_lang("Duration") . ' : </span><span>' . $duration . '</span><br/>';
    }
    $result .='';
    $result .='</div>';
    ...
    if ($Closed == '') {
        ...
        // We check course criteria
       $onclick = '';
       $courseUrl = api_get_path(WEB_COURSE_PATH) . $course_directory . '/?id_session=' . $my_course['id_session'];
       if (!empty($course_info['course_criteria'])) {
            $is_criteria_done = api_is_user_course_exam_done(api_get_user_id(), $course_info['course_criteria'], api_get_session_id());                        
            if (!api_is_allowed_to_create_course() && !$is_criteria_done) {
                $criteriaInfo = api_get_course_info($course_info['course_criteria']);
                $msgCriteria = sprintf(get_lang('CourseCriteriaXNotAllowed'), '<b>'.$criteriaInfo['name'].'</b>');
                $courseUrl = 'javascript:void(0);';
                $onclick = '$.alert(\''.$msgCriteria.'\', \''.get_lang('Error').'\', \'error\')';
            }
        }        
        $result .= '<p class="listItem__btn"><a href="' .$courseUrl. '" onclick="'.$onclick.'" class="btn" style="right:0px;bottom:0px;"><span>' . get_lang('Enter') . '</span></a></p>';
    }
...
echo '<div id="" style="min-height:1px;">';

// Main left menu
echo '<div class="left__content sidebar sidebar--cursus" style="' . $none . '">';
// Left menu
echo '<div id="menu" class="sidebar__block block block--list" >';
...
if ((api_is_platform_admin() || api_is_course_admin() || api_is_allowed_to_create_course()) && ($_SESSION["studentview"] != "studentenview")) {
    $show_menu = true;
}
...
// My account section
if ($show_menu) {
    echo api_display_tool_title(get_lang('MenuUser'), 'block__title');
    ...
    echo '<script type="text/javascript">$(document).ready(function(){ /*alert("yes");*/$("#main__content").css("width","98%");$("#main__content").css("padding","0 17px")});</script>';
}
...
echo '<div class="main__content content" id="main__content" style="float:right">';
if (isset($_GET['history']) && intval($_GET['history']) == 1) {
...
if (empty($show_list_course) && empty($show_list_sessions) && empty($show_list_categories_sessions)) {
    ...
    $check = api_get_show_catalogo_tab();
    $mutiLanguage = (api_get_setting("multi_language") === 'true') ? true : false;

    if($check AND $mutiLanguage){
        $list_category = get_list_category_courses($_POST);
        echo $list_category;
    }
...
///function list
function get_list_category_courses($POST){
    $category = isset($POST['category_list']) ? $POST['category_list'] : null; 
    $objCatalog = new EcommerceCatalog();
    $Categories = $objCatalog->getListCategoryCourse(true,true,true);    
    
    $user_id = api_get_user_id();
    $user_info = api_get_user_info($user_id);
    $language = $user_info['language'];
    $html = '';    
    //$html .= '<form id="listcategory" name="listcategory" method="post">';
    //$html .= '<select id="category_list" class="chzn-select chzn-done"  name="category_list">';
    //$html .= '<option value ="ALL">--- '.get_lang('All'). '---</option>';
    foreach($Categories as $id => $value){
        $select = ($value['code_category']== $category) ? 'selected' : '';
            //$html .= '<option value ="'.$value['code_category'].'" '.$select.'>';
            //$html .= cut($value['name'],40);
            //$html .= '</option>';
    }
    //$html .= '<select><span id="ddlspan">'.get_lang("Category").'</span>';
    //$html .= '</form>';
   // $html .= '<div class="clear_b"></div>';
    $hmlt ='';
    return $html;
}
function show_list_course($history, $category_courses, $category_sessions) {
    ...
    if (!isset($history) && count($category_courses) > 0) {
        ...
        $mutiLanguage = (api_get_setting("multi_language") === 'true') ? true : false;
        if($check AND $mutiLanguage){
            $html .=  '<span><h1 class="title--page " style="margin-bottom: 0px;">'.get_lang("IAmRegisteredTo")." ".count( CourseManager::get_courses_list_by_user_id($_user['user_id']))." ".get_lang("Courses").'</h1></span>';
        }
        ...
        foreach ($category_courses as $course) {//List Courses
            ...
            $c = get_logged_user_course_html($course, 0, 'clearfix cursus__content-item listItem');
            $html .='<li>' . $c[1] . '</li>';
        }
    ...
    if (count($category_sessions) > 0) {
        ...
        foreach ($category_sessions as $session) {
            ...
            if (!api_is_allowed_to_create_course() && (isset($check['started']) && $check['started'] == false)) {
                ...
                $html .= '<div class="clearfix cursus__content-item">';
...
$( document ).ready(function() {
    $("#category_list").change(function(){
        ...
        $.ajax({
            url:'<?php echo  api_get_path(WEB_AJAX_PATH); ?>courseslist.ajax.php?action=getcourses&id='+selectValue,
            type:'POST',                                                            
            success: function(data){
                $( ".courseslist li" ).remove();
                $( ".courseslist" ).html(data);
            },
            beforeSend : function (){
                $( ".courseslist li" ).remove();
                $( ".courseslist" ).html('<li style="text-align: center;margin:20px;"><img src="<?php echo api_get_path(WEB_IMG_PATH).'navigation/ajax-loader.gif';  ?>" /></li>');
            }
        })
    })
})
